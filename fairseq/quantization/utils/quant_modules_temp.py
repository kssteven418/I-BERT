...
class QuantAct(Module):
    ...
    def forward(self, x, 
        ...
        if pre_act_scaling_factor is None:
            ...
            if debug:
                print('scale factor', self.act_scaling_factor)
...
class QuantGELU(Module):
    def __init__(self,
        ...
        self.slope_tail = 0.390
        self.clamp = 4.04
        self.slope_mid = 4 * self.slope_tail
        self.cut = (1 - self.slope_tail) * self.clamp / (3 * self.slope_tail) # x-coordinate where the slope changes

        self.y_axis = self.clamp - self.slope_tail * self.clamp
        self.x_axis = -self.y_axis / self.slope_tail
    ...
    def sigmoid_approx(self, x_int, scaling_factor):
        with torch.no_grad():
            ...
            clamp_int2= torch.floor(self.clamp / (scaling_factor * self.slope_tail))
            cut_int = torch.floor(self.cut / scaling_factor)
            x_axis_int = torch.floor(self.x_axis / scaling_factor)

        x_int = torch.min(torch.max(x_int, -clamp_int), clamp_int)
        x_small = x_int.lt(-cut_int)
        x_large = x_int.gt(cut_int)
        x_mid = (x_large | x_small).logical_not()

        x_int = x_mid * 4. * x_int + \
            x_small * (x_int + x_axis_int) + \
            x_large * (x_int - x_axis_int)

        x_int = x_int + clamp_int2

        scaling_factor = scaling_factor * self.slope_tail / (2 * self.clamp)
...
class QuantSoftmax(Module):
    def __init__(self,
        ...
        self.xlow = -5.6
        self.xmid = -2.5
        self.base_slope = Parameter(torch.tensor(0.018))
    ...
    def exp_segment(self, x_int, scaling_factor):
        self.initialize()
        print(self.base_slope)

        xlow_int = int(self.xlow / scaling_factor)
        xmid_int = floor_ste.apply(self.xmid / scaling_factor)
        xhigh_int = floor_ste.apply(self.xhigh / scaling_factor)
